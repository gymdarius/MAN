    ...
    def train(self, num_step):
        ...
        for i in range(num_step):
            ...
            if self.c % self.report_freq == 0:
                ...
                print(f"Step: {self.c}, Reward mean: {reward_mean:.2f}, Reward std: {reward_std:.2f}, Max_height mean: {max_height_mean:.2f}, Max_height std: {max_height_std:.2f}, Bumpiness mean: {bumpiness_mean:.2f}, Bumpiness std: {bumpiness_std:.2f}, Loss: {total_loss / self.c:.4f}")
...
def test(env_name, model, model_file=None):
    ...
    with torch.no_grad():
        ...
        total_rewards, total_max_heights, total_bumpiness = [], [], []
        ...
        for _ in range(test_trial):
            ...
            while True:
                ...
                if done:
                    ...
                    virance = sum(torch.abs(next_state[0, i + 1] - next_state[0, i]) for i in range(next_state.size(1) - 1))
...
def main(args):
    ...
    reward_means_total, reward_stds_total, max_height_means_total, max_height_stds_total, bumpiness_means_total, bumpiness_stds_total = [], [], [], [], [], []
